...
\end{align}
for all $r$, where $0<r<1$.
Then, $g(t)$ must be $0$ for all $t\ge n$
(Argument as below, not sure how to prove this concisely:
The convergence radius of the last power series is at least $1$.
Then we can extend this power series into an complex analytic function
at the open ball $|z|<1$.
Since zeros of a non-constant analytic function are isolated.
This function must be the constant $0$.
So the power series must have $0$ as coefficients
).
