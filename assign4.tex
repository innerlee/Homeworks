...
\subsubsection{} %4.16.1

$X_i$ can be seen as first drawing from an exponential distribution with
parameter $\theta_2$,
...
\begin{equation}
    g_{\theta_1,\theta_2} (y) = \frac{1}{n\theta_2}\exp\bigg(-\frac{y-\theta_1}{n\theta_2}\bigg)
\end{equation}
...
\begin{align}
    &f_{\theta_1,\theta_2}\Big(X_{(1)}=y, \sum_{i=1}^n X_i-X_{(1)}=x\Big) \\
        ...
        &= \frac{1}{n\theta_2}e^{-\frac{y-\theta_1}{n\theta_2}}
        \frac{1}{\Gamma(n-1)\theta_2^{n-1}}x^{n-2} e^{-\frac{x}{\theta_2}} \\
        &= \frac{x^{n-2}}{n\Gamma(n-1)\theta_2^n}e^{\frac{\theta_1}{n\theta_2}}e^{-\frac{x}{\theta_2}-\frac{y}{n\theta_2}}
\end{align}
It belongs to exponential family,
so $(y, x)=\big(X_{(1)}, \sum_{i=1}^n X_i-X_{(1)}\big)$ is complete.

\subsubsection{} %4.16.2

Obvious from Equation~\eqref{eq:116}.

\subsubsection{} %4.16.3
...
So, UMVUE for $\theta_1$ is $X_{(1)} - \frac{n}{n-1}\sum_{i=1}^n\big(X_i-X_{(1)}\big)$,
