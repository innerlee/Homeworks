...
% \begin{align}
%     & P(X_{(m)}/X_{(m+1)}\le t) \\
%         &= n! P(X_{(m)}/X_{(m+1)}\le t, X_{(1)}=X_1, \dots, X_{(n)}=X_n) \\
%         &= n! P(X_{(1)}=X_1, \dots, X_{(n)}=X_n) P(X_{(m)}/X_{(m+1)}\le t ~|~ X_{(1)}=X_1, \dots, X_{(n)}=X_n) \\
%         &= P(X_{(m)}/X_{(m+1)}\le t ~|~ X_{(1)}=X_1, \dots, X_{(n)}=X_n) \\
%         &= \int_0^\theta P(X_m \le tu, X_{m+1}=u ~|~ X_{(1)}=X_1, \dots, X_{(n)}=X_n) du
%         &= \int_0^\theta P(X_m \le tu, X_{m+1}=u ~|~ X_{(1)}=X_1, \dots, X_{(n)}=X_n) du
% \end{align}
